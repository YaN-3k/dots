...
#define SEARCH { \
        .v = (const char *[]){ "/bin/sh", "-c", \
	     ...
	     "cat ~/.local/share/bookmarks) " \
             "| xargs -0 printf %b | dmenu -w $0 `\" &&" \
...
#define SR_SEARCH { \
	.v = (const char *[]){ "/bin/sh", "-e", "-c", \
		"elvi=$(sr -elvi | tail -n +2 | cut -s -f1 | dmenu -w $0);" \
...
#define BM_ADD(r) {\
        .v = (const char *[]){ "/bin/sh", "-c", \
             ...
             "| sed 's/https\\?:\\/\\/\\(www\\.\\)\\?//' && cat ~/.local/share/bookmarks) " \
             "| awk '!seen[$0]++' > ~/.local/share/bookmarks.tmp && " \
             "mv ~/.local/share/bookmarks.tmp ~/.local/share/bookmarks", \
             winid, r, NULL \
        } \
}
